 ...
 *        resets the envelope positions. (Minor: ft2-clone ramps the env.)
 * 14-24: Instrument number: ED0 is ignored, ED1/ED2 with keyoff
 *        resets the envelope positions. (Minor: ft2-clone does not ramp.)
...
TEST(test_player_ft2_delay_envelope_on)
{
	compare_mixer_data(
		"data/ft2_delay_envelope_on.xm",
		"data/ft2_delay_envelope_on.data");
}
END_TEST
