...
TEST(test_no_note_same_ins_ft2)
{
	...
	new_event(ctx, 0, 3, 0, 0,            0,     0, FX_VOLSET, SET_VOL,
							FX_SETPAN, SET_PAN);
	new_event(ctx, 0, 4, 0, XMP_KEY_FADE, INS_0, 0, 0x00, 0, 0, 0);
	...
	/* Row 4: same instrument with key off (loaded as XMP_KEY_FADE) (FT2)
	 *
	 * This behaves the same as same instrument with no note.
	 */
	xmp_play_frame(opaque);
	check_on(xc, vi, KEY_C5, INS_0,
		 -1 /*FIXME: INS_0_SUB_0_VOL*/, INS_0_SUB_0_PAN, INS_0_FADE, "row 4");
